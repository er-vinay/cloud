<?php
	...
	class TaskController extends CI_Controller
	{
		...
		public function index($stage)
		{
		    ...
		    $config = array();
			$config["base_url"] = $url;
			$config["total_rows"] = $this->Tasks->countLeads($conditions);
			$config["per_page"] = 10;
	  ...
	  //   }
        ...
        public function sanctionleads()
        {
			...
			if ($this->input->server('REQUEST_METHOD') == 'POST') 
		    {
		        ...
		        $cam = $sql2->row();
	        	if(($sql2->num_rows() == 0)) {
	        		$json['err'] = "CAM Fields required, Please re-check";
		            echo json_encode($json);
